...
function ProductList({ onHomeClick }) {
  ...
  const handleAddToCart = (item) => {
    ...
    console.log("them item thanh cong");
  };

  const checkInCart = (item) => {
    const isExisted = cart.find((i) => i.name === item.name);
  ...
  return (
    <div>
      <div className="navbar" style={styleObj}>
        <div className="tag">
          <a href="/" className="luxury" onClick={(e) => handleHomeClick(e)}>
            <img
        ...
        <div className="product-grid">
          {plantsArray.map((category, index) => (
            <div key={index} className="product-grid">
              <h2 className="product-category">{category.category}</h2>
              <div className="product-list">
                {category.plants.map((item, i) => (
                  <div key={i} className="product-card">
                    ...
                    <button
                      className={`product-button ${
                        checkInCart(item) ? "added-to-cart" : ""
                      }`}
                      onClick={() => {
                        handleAddToCart(item);
                      }}
                      disabled={checkInCart(item)}
                    >
                      Add to Cart
                    </button>
                  </div>
